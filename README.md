...
| 缓存/并发 | Redis（String）+ Lua 脚本、乐观锁版本号 |
| 测试 | Postman、JMeter |
...
2. **数据库乐观锁兜底（最终保证）**：落库时用 `version` 条件更新
   `UPDATE ... SET stock=stock-1, version=version+1 WHERE id=? AND version=? AND stock>0`；
   只有真正扣减成功才写订单，**数据库层面绝不超卖**。
3. **接口限流（简单固定窗口）**：Redis 计数器限制"同一用户 1 秒 1 次"，防恶意刷单。
4. **失败补偿**：若 Redis 预减成功但数据库乐观锁扣减失败（被他人抢先/抢光），
   会把 Redis 里预减的库存 `INCR` 回补，保持 Redis 与 DB 一致。

> 一句话讲清：**Redis 是"快速拦截 + 扛流量"；数据库乐观锁是"最终一致 + 不超卖"的兜底。**
